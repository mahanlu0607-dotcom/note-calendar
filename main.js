/* THIS IS A GENERATED/BUNDLED FILE BY ESBUILD, DO NOT EDIT */
...
function getH1Title(app, file, cache) {
  try {
...
function tintForCard(hex) {
  const r = parseInt(hex.substring(1, 3), 16);
  const g = parseInt(hex.substring(3, 5), 16);
  const b = parseInt(hex.substring(5, 7), 16);
  const mix = (c) => Math.round(255 - (255 - c) * 0.12);
...
var _Solar = class {
  ...
  nextDay(days) {
    ...
    if (days > 0) {
      d += days;
      let daysInMonth2 = SolarUtil.getDaysOfMonth(y, m);
      while (d > daysInMonth2) {
        ...
        daysInMonth2 = SolarUtil.getDaysOfMonth(y, m);
      }
    } else if (days < 0) {
      while (d + days <= 0) {
        m--;
...
function lunarText(y, m, d) {
  try {
    ...
    return `${ganZhi}${shengXiao}\u5E74 \xB7 \u519C\u5386${month}\u6708${day}`;
  } catch (e) {
    return "";
  }
}
...
// src/calendar-view.ts
var CAL_VIEW_TYPE = "note-calendar-view";
var DRAG_MIME = "application/x-note-calendar";
var MONTH_NAMES = ["\u4E00\u6708", "\u4E8C\u6708", "\u4E09\u6708", "\u56DB\u6708", "\u4E94\u6708", "\u516D\u6708", "\u4E03\u6708", "\u516B\u6708", "\u4E5D\u6708", "\u5341\u6708", "\u5341\u4E00\u6708", "\u5341\u4E8C\u6708"];
var WEEK_HEAD = ["\u5468\u4E00", "\u5468\u4E8C", "\u5468\u4E09", "\u5468\u56DB", "\u5468\u4E94", "\u5468\u516D", "\u5468\u65E5"];
var WEEK_HEAD_SHORT = ["\u4E00", "\u4E8C", "\u4E09", "\u56DB", "\u4E94", "\u516D", "\u65E5"];
...
var CalendarView = class extends import_obsidian.ItemView {
  ...
  get isMobile() {
    return import_obsidian.Platform.isMobile || this.plugin.settings.simulateMobile;
  }
  getViewType() {
  ...
  renderMonth(container) {
    ...
    const slider = tool.createEl("input", { type: "range", cls: "nc-slider" });
    slider.min = "80";
    slider.max = "150";
    ...
    const addCell = (k, dayNum, adj, isToday) => {
  ...
  rerenderMonthCards() {
    ...
    cells.forEach((cellEl) => {
      const cards = cellEl.querySelector(".nc-cards");
      const dk = cellEl.dataset.dateKey;
      if (!cards || !dk)
        return;
      cards.empty();
  ...
  showYearMonth(container, y, m) {
    let panel = container.querySelector(".nc-ympanel");
    if (!panel)
      panel = container.createDiv({ cls: "nc-ympanel" });
    panel.empty();
    ...
    panel.createDiv({ cls: "nc-ympanel-title", text: `${y}\u5E74${MONTH_NAMES[m - 1]} \xB7 ${filtered.length} \u4E2A\u6587\u4EF6` + (this.yearFilter !== "all" ? `\uFF08\u4EC5${this.yearFilter}\uFF09` : "") });
  ...
  renderEntries(container, entries, dateKeyStr) {
    if (entries.length === 0)
      return;
    const cellH = this.plugin.settings.cellHeight;
    const PER = 14;
  ...
  showCardMenuAt(x, y, e) {
    const menu = new import_obsidian.Menu();
    menu.addItem(
      (item) => item.setTitle("\u6253\u5F00\u7B14\u8BB0").setIcon("document").onClick(() => this.openNote(e.path))
    );
    menu.addItem(
      (item) => item.setTitle("\u4FEE\u6539\u65E5\u671F\u2026").setIcon("calendar").onClick(() => this.plugin.promptSetDate(e))
    );
    if (this.isMobile) {
      menu.addItem(
        (item) => item.setTitle("\u79FB\u52A8\u5230\u65E5\u671F\u2026").setIcon("forward").onClick(() => this.moveNoteToDatePicker(e.path))
      );
    }
    menu.addItem(
      (item) => item.setTitle("\u6E05\u9664\u624B\u52A8\u65E5\u671F\uFF08\u56DE\u5230\u6587\u4EF6 ctime\uFF09").setIcon("reset").onClick(async () => {
        this.plugin.store.setManualDate(e.path, void 0);
    ...
    menu.addItem(
      (item) => item.setTitle("\u5728\u6587\u4EF6\u5939\u4E2D\u663E\u793A").setIcon("folder").onClick(() => {
        const leaf = this.app.workspace.getLeaf(true);
        leaf.openFile(this.app.vault.getAbstractFileByPath(e.path));
  ...
  bindDateCell(cell, dateKeyStr) {
    ...
    if (!this.isMobile) {
      ...
      cell.addEventListener("drop", async (ev) => {
        ...
        ev.preventDefault();
        cell.removeClass("nc-drop-target");
        const raw = (_a = ev.dataTransfer) == null ? void 0 : _a.getData(DRAG_MIME);
        if (!raw)
          return;
        try {
          const data = JSON.parse(raw);
          if (data.from === dateKeyStr)
            return;
          await this.plugin.moveNoteToDate(data.path, dateKeyStr);
          new import_obsidian.Notice(`\u5DF2\u79FB\u52A8\u5230 ${dateKeyStr}`);
        } catch (err) {
        }
      });
    }
  ...
  /** 移动端"移动到日期"选择弹窗 */
  async moveNoteToDatePicker(path) {
    const { Modal: Modal2, Setting: Setting3 } = await import("obsidian");
    class MoveModal extends Modal2 {
      ...
      onOpen() {
        ...
        const picker = contentEl.createEl("input", { type: "date" });
    ...
    new MoveModal(this.app, async (date) => {
      ...
      new import_obsidian.Notice(`\u5DF2\u79FB\u52A8\u5230 ${date}`);
    }).open();
  }
  showDateMenu(ev, dateKeyStr) {
    this.showDateMenuAt(ev.clientX, ev.clientY, dateKeyStr);
  }
  /** 弹日期菜单（桌面右键 / 移动长按共用） */
  showDateMenuAt(x, y, dateKeyStr) {
    const menu = new import_obsidian.Menu();
    menu.addItem(
      (item) => item.setTitle("\u65B0\u5EFA\u7B14\u8BB0").setIcon("plus").onClick(() => this.createNoteOnDate(dateKeyStr))
    );
    menu.showAtPosition({ x, y });
  }
  async createNoteOnDate(dateKeyStr) {
...
var DateStore = class {
  ...
  getDateForFile(file) {
    ...
    try {
      ...
      if (stat instanceof import_obsidian2.TFile && stat.stat) {
...
var GraphColorSync = class {
  ...
  async load() {
    try {
      ...
      this.colorGroups = groups.filter((g) => typeof (g == null ? void 0 : g.query) === "string" && g.query.trim().startsWith("path:")).map((g) => {
        ...
        if (typeof color.rgb === "number") {
          ...
          hex = "#" + [toHex(color.r), toHex(color.g), toHex(color.b)].map((v) => v.toString(16).padStart(2, "0")).join("").toUpperCase();
        }
  ...
  getColor(folder) {
    ...
    return hit == null ? void 0 : hit.hex;
  }
  getAllGroups() {
    return this.colorGroups.slice();
  }
  async setColor(folder, hex) {
    if (!this.initialized)
      await this.load();
  ...
  async writeBack() {
    try {
      ...
      const groups = (Array.isArray(json.colorGroups) ? json.colorGroups : []).filter(
        (g) => !(typeof (g == null ? void 0 : g.query) === "string" && g.query.trim().startsWith("path:"))
      );
...
var NoteCalendarSettingTab = class extends import_obsidian3.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("\u9ED8\u8BA4\u89C6\u56FE").setDesc("\u6253\u5F00\u65E5\u5386\u65F6\u7684\u521D\u59CB\u89C6\u56FE").addDropdown(
      (dd) => dd.addOption("m", "\u6708\u89C6\u56FE").addOption("y", "\u5E74\u89C6\u56FE").setValue(this.plugin.settings.defaultView).onChange(async (v) => {
        this.plugin.settings.defaultView = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian3.Setting(containerEl).setName("\u6A21\u62DF\u79FB\u52A8\u7AEF\u754C\u9762").setDesc("\u5728 Mac \u4E0A\u9884\u89C8\u624B\u673A\u7AEF\u6548\u679C\uFF08\u624B\u673A\u7AEF\u5E03\u5C40\u3001\u957F\u6309\u83DC\u5355\u3001\u6ED1\u52A8\u5207\u6708\uFF09\u3002\u5173\u95ED\u540E\u6062\u590D\u684C\u9762\u7AEF\u3002").addToggle(
      (tg) => tg.setValue(this.plugin.settings.simulateMobile).onChange(async (v) => {
        this.plugin.settings.simulateMobile = v;
        await this.plugin.saveSettings();
        this.plugin.refreshAllViews();
      })
    );
    new import_obsidian3.Setting(containerEl).setName("\u6708\u89C6\u56FE\u5361\u7247\u95F4\u8DDD").setDesc("\u6708\u4EFD\u683C\u5B50\u9AD8\u5EA6\uFF08\u8D8A\u5927\u5361\u7247\u95F4\u8DDD\u8D8A\u5927\uFF09").addSlider(
      (sl) => sl.setLimits(80, 150, 2).setValue(this.plugin.settings.cellHeight).setDynamicTooltip().onChange(async (v) => {
        this.plugin.settings.cellHeight = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian3.Setting(containerEl).setName("\u672A\u914D\u7F6E\u6587\u4EF6\u5939\u7684\u9ED8\u8BA4\u989C\u8272").setDesc("\u56FE\u8C31\u4E2D\u6CA1\u6709\u8BBE\u7F6E\u989C\u8272\u7684\u6587\u4EF6\u5939\uFF0C\u5361\u7247\u4F7F\u7528\u6B64\u989C\u8272").addText(
      (t) => t.setValue(this.plugin.settings.fallbackHex).onChange(async (v) => {
    ...
    for (const g of groups) {
      const row = new import_obsidian3.Setting(containerEl).setName(g.query).setDesc(`path:${g.query}  \xB7 \u4E0E\u5173\u7CFB\u56FE\u8C31\u540C\u6B65`).addColorPicker(
        (cp) => cp.setValue(g.hex).onChange(async (val) => {
          await this.plugin.setFolderColor(g.query, val);
          this.display();
        })
      ).addExtraButton(
        (btn) => btn.setIcon("trash").setTooltip("\u5220\u9664\u8BE5\u6587\u4EF6\u5939\u7684\u989C\u8272\uFF08\u540C\u65F6\u4ECE\u56FE\u8C31\u79FB\u9664\uFF09").onClick(async () => {
    ...
    new import_obsidian3.Setting(containerEl).setName("\u6DFB\u52A0\u6587\u4EF6\u5939\u989C\u8272").setDesc("\u9009\u62E9 vault \u4E2D\u7684\u4E00\u4E2A\u6587\u4EF6\u5939\u5E76\u6307\u5B9A\u989C\u8272").addDropdown((dd) => {
      ...
      return dd;
    }).addColorPicker((cp) => cp.setValue("#5F5E5A")).addButton(
      (btn) => btn.setButtonText("\u6DFB\u52A0").onClick(async () => {
        const ddEl = btn.buttonEl.closest(".setting-item").querySelector("select");
        const folder = ddEl.value;
        if (!folder)
          return;
        const cpEl = btn.buttonEl.closest(".setting-item").querySelector("input[type=color]");
...
// src/main.ts
var NoteCalendarPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    this.addRibbonIcon("calendar-with-checkmark", "\u6253\u5F00\u7B14\u8BB0\u65E5\u5386\uFF08\u53F3\u952E\u9009\u4F4D\u7F6E\uFF09", (ev) => {
      if (ev && ev.button !== void 0 && ev.button !== 0)
        return;
      this.openView("right");
    });
    ...
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        if (file instanceof import_obsidian4.TFile && file.extension === "md") {
          menu.addItem(
            (item) => item.setTitle("\u4FEE\u6539\u65E5\u5386\u65E5\u671F\u2026").setIcon("calendar").onClick(() => this.promptSetDateForPath(file.path))
          );
        }
      })
    );
  ...
  entriesForDate(dateKeyStr) {
    return this.store.getByDate(dateKeyStr);
  }
  monthHasNotes(y, m) {
    const dim = new Date(y, m, 0).getDate();
    for (let d = 1; d <= dim; d++) {
      if (this.store.getByDate(dateKey(y, m, d)).length > 0)
        return true;
    }
    return false;
  }
  // ============ 颜色 ============
  colorForFolder(folder) {
  ...
  async setFolderColor(folder, hex) {
  ...
  async openView(side) {
    ...
    if (existing.length > 0) {
      ...
      leaf = this.app.workspace.getLeaf(side);
    }
    if (existing.length === 0) {
  ...
  injectRightRibbonIcon() {
    ...
    const doInject = () => {
      for (const sel of candidates) {
        ...
        if (!container)
          continue;
        if (container.querySelector(`.${CAL_ID}`))
          return;
  ...
  promptSetDateForPath(path) {
    const current = this.store.getDateForFile(this.app.vault.getAbstractFileByPath(path));
...
var DateEditModal = class extends import_obsidian4.Modal {
